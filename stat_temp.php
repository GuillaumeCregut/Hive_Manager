...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * 																	 *
 * Nom de la page :	stat_temp.php									 *
 * Date création :	02/03/2017										 *
 * Date Modification : 08/03/2017									 *
 * Créateur : Guillaume Crégut										 *													
 * Version :	0.2A												 *
 * Objet et notes :													 *
 *	Statistiques des relevés température en utilisant les			 *
 *	visites et temp ext datalogger									 *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
 session_start();
...
*/
			...
			if(isset($_POST['Rech_temp']))
			{
				...
				else
				{
					...
					$SQLS="SELECT count(*) as Compte from visite WHERE temperature>-100 AND ID_Ruche=$Id_Ruche AND Date_Visite BETWEEN '$DateDebut' AND '$DateFin'";
					$Compte=0;
					foreach($DBConn->query($SQLS) as $row)
					{
						$Compte=$row['Compte'];  
					...
					else
					{
						...
						$SQLS="SELECT temperature,Date_Visite  from visite WHERE temperature>-100 AND ID_Ruche=$Id_Ruche AND Date_Visite BETWEEN '$DateDebut' AND '$DateFin'";
						$i=0;
						$TabVisiteTemp=array();
						foreach($DBConn->query($SQLS) as $row)
						{
						...
						$SQLS="SELECT Date_log,TempExt FROM datalogger WHERE ID_RUCHE=$Id_Ruche AND (Date_log BETWEEN '$DateDebut' AND '$DateFin') AND TempExt>-100";
						$i=0;
						$TabDataLogTemp=array();
						foreach($DBConn->query($SQLS) as $row)
						{
							$TabDataLogTemp[$i]['Date']=$row['Date_log'];
							$TabDataLogTemp[$i]['valeur']=$row['TempExt'];
							$i++;
						}
						...
						array_multisort($Date,SORT_ASC,$Temperature,SORT_ASC,$TabTotal);
						//Génération du graphe
						$Titre="Relevé des températures de la ruche '$NomRuche' sur la période du $DateDebutAff au $DateFinAff";
			...
			else
			{
				...
				$Page=$CheminTpl.'stat_ruche_rech_temp.tpl';
				$template->display($Page);
			}
