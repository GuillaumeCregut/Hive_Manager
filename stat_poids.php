...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * 																	 *
 * Nom de la page :	stat_poids.php									 *
 * Date création :	03/03/2017										 *
 * Date Modification :	08/03/2017									 *
 * Créateur : Guillaume Crégut										 *													
 * Version :	0.2A												 *
 * Objet et notes :													 *
 *	Statistiques des relevés du poids en utilisant les				 *
...
*/
			...
			if(isset($_POST['Rech_temp']))
			{
				...
				else
				{
					...
					$SQLS="SELECT count(*) as Compte from visite WHERE POIDS>0 AND ID_Ruche=$Id_Ruche AND Date_Visite BETWEEN '$DateDebut' AND '$DateFin'";
					$Compte=0;
					foreach($DBConn->query($SQLS) as $row)
					{
						$Compte=$row['Compte'];  
					...
					else
					{
						...
						$SQLS="SELECT POIDS,Date_Visite  from visite WHERE POIDS>0 AND ID_Ruche=$Id_Ruche AND Date_Visite BETWEEN '$DateDebut' AND '$DateFin'";
						$i=0;
						$TabVisiteTemp=array();
						foreach($DBConn->query($SQLS) as $row)
						{
							$TabVisiteTemp[$i]['Date']=$row['Date_Visite'];
							$TabVisiteTemp[$i]['valeur']=$row['POIDS'];
							$i++;
						}
						//On récupère les mesures du datalogger
						$SQLS="SELECT Date_log,Poids FROM datalogger WHERE ID_RUCHE=$Id_Ruche AND (Date_log BETWEEN '$DateDebut' AND '$DateFin') AND Poids>0";
						$i=0;
						$TabDataLogTemp=array();
						foreach($DBConn->query($SQLS) as $row)
						{
							$TabDataLogTemp[$i]['Date']=$row['Date_log'];
							$TabDataLogTemp[$i]['valeur']=$row['Poids'];
							$i++;
						}
			...
			else
			{
				...
				$Page=$CheminTpl.'stat_ruche_rech_pesee.tpl';
				$template->display($Page);
			}
		}
		else
